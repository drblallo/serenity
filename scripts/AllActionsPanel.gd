...
func _ready() -> void:
	...
	Rules.on_state_changed.connect(on_state_change)
	on_state_change()
...
func on_state_change():
	...
	clear_actions()
	var counter = 0
	for action in Rules.valid_actions:
		add_action(Rules.strip_symbols(Rules.as_str(action)), action, counter)
		counter = counter + 1
